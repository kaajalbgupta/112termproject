...
def appStarted(app):
    ...
    app.players[app.playersToBeInnocent[0]].smileAfterMurderNumber=1
    app.players[app.playersToBeInnocent[1]].smileAfterMurderNumber=2
    ...
    app.cakeImage=app.scaleImage(app.loadImage('https://i.ibb.co/X45BXd5/IMG-16-AF06-F73-E50-1.jpg'), 2/5)
    app.bloodImage=app.scaleImage(app.loadImage('https://i.ibb.co/1swdmVm/IMG-6-CFD62-D7-B854-1.jpg'), 3/10)
    app.backImage=app.scaleImage(app.loadImage('https://i.ibb.co/gW0XyDR/baseline-arrow-back-white-18dp.png'), 2/3)
    app.bannerImage=app.scaleImage(app.loadImage('https://i.ibb.co/mTcP97B/IMG-EF19-D540867-A-1-removebg-preview.png'), 4/5)
...
def updateMockProtagList(app, newInformation, numMurder, murderTime, protag):
    if(numMurder>0):
        murder=app.villain.murders[numMurder-1]
        for uid in app.players:
            player=app.players[uid]
            if(not player.alive or player.eliminated):
                protag.innocents.add(player.uid)
    ...
    if(murderTime):
        ...
        if(murder.room!=room):
            otherPeopleInRoom=getPeopleInRoom(app, room)
            for personUid in otherPeopleInRoom:
                protag.innocents.add(personUid)
    ...
    for info in newInformation:
        ...
        if(player.alive and not player.eliminated):
            if(info.innocent==True):
                protag.innocents.add(info.personUid)
            elif(info.room==murder.room and info.personUid not in protag.innocents):
                protag.suspects.add(info.personUid)
            elif(info.personUid not in protag.innocents and info.personUid not in protag.suspects):
                protag.unknowns.add(info.personUid)
    
    protag.suspects=protag.suspects.difference(protag.innocents)
    protag.unknowns=protag.unknowns.difference(protag.innocents)
    protag.unknowns=protag.unknowns.difference(protag.suspects)
...
def checkIfInfoRevealsMurderer(app, knownInformation, numMurder, players, murders, protag):
    murder=murders[numMurder]
    innocents, suspects, unknowns=copy.deepcopy(protag.innocents), copy.deepcopy(protag.suspects), copy.deepcopy(protag.unknowns)
    ...
    if((len(suspects)==1 and app.villain.uid in suspects and len(unknowns)==0) or (len(unknowns)==1 and len(suspects)==0 and app.villain.uid in unknowns)): 
        return True
    else: return False
...
def getPathsForPlayerToFindMurderer(app, currInfo, playersDone, currMurder, players, murders, protag):
    #Base Case
    if(checkIfInfoRevealsMurderer(app, currInfo, currMurder, players, murders, protag)):
        return playersDone
            ...
            if(players[uid].alive and uid not in playersDone):
                ...
                if(len(players[uid].information)>currMurder):
                    ...
                    for info in information:
                        ...
                        returned=getPathsForPlayerToFindMurderer(app, newCurrInfo, newPlayersDone, currMurder, players, murders, protag)
    ...
    for uid in players:
        if(players[uid].alive):
            ...
            paths=getPathsForPlayerToFindMurderer(app, [], 
                [], app.numMurder, players, newMurders, app.protag)
            ...
            if(paths==None or len(paths)==0):
                ...
                if(minPathSize==0 or currMinPathSize>=minPathSize):
                    if(currMinPathSize==minPathSize):
                        if(uidToKill==None or players[uid].characteristics.distractedness<players[uidToKill].characteristics.distractedness):
...
def kill(app, personUid):
    ...
    print(f"room to go: {roomToGo}")
    if(app.room!=roomToGo):
        app.murderExistence+=1
        print(f"murder existence: {app.murderExistence}")
        if(app.murderExistence==2):
            app.lightsOff=True
            ...
            for room in range(0, 5):
                uids=getPeopleInRoom(app, room)
                names=[]
                if(app.villain.uid in uids):
                    names+=[app.villain.name]
                    uids.remove(app.villain.uid)
                for uid in uids:
                    names+=[app.players[uid].name]
                print(f"ROOM: {room}, PLAYERS: {names}")
            app.victimUid=None
            app.lightsOffTime=0
            if(app.numMurder==5):
                app.gameOver=True
...
def movePlayerTowardsRoom(app, player):
    if(player.room!=player.gravityRoom):
        if(player.position<=app.border+app.radius):
            if(player.room!=0):
                player.room-=1
                player.position=app.width-app.border-app.radius-4
        elif(player.position>=app.width-app.border-app.radius):
            if(player.room!=4):
                player.room+=1
                player.position=app.border+app.radius+4
        else:
...
def playerCollectInformation(app, playerUid, isMurderTime):
    ...
    if(playerUid in app.players):
        numEventsRecorded=roundHalfUp(numEventsRecorded*(1-player.characteristics.distractedness))
        if(len(events)>0): print(numEventsRecorded, len(events))
    ...
    if(app.numMurder in player.information):
        ...
        if(playerUid in app.players): 
            app.players[playerUid].information[app.numMurder]=[]
            app.players[playerUid].information[app.numMurder]=eventsRecorded
        else: 
            app.villain.information[app.numMurder]=[]
            app.villain.information[app.numMurder]=eventsRecorded
...
def timerFired(app):
    if(not app.paused and app.pressed and not app.showChat):
        ...
        if(app.mouseY>=app.border+100):
            if(app.mouseX>app.protag.position and 
                ...
                app.protag.position+=(difference/120)*7
    ...
    if(not app.paused):
            ...
            if(not player.stationary and player.alive):
                ...
                # move around 180 px near gravity
                    ...
                    if(player.currFriendIndex!=-1):
                        ...
                        diff=random.choice([50, -50])
                        # sharing information
                        shareInfo=random.randint(0, 4)
                        if(app.deciseconds%250==0 and shareInfo==0 and app.numMurder in player.information and len(player.information[app.numMurder])>1):
                            print("sharing")
                            information=player.information[app.numMurder]
                            info=random.choice(information)
                            if(uid in app.players): 
                                if(app.numMurder in app.players[uid].information):
                                    app.players[uid].information[app.numMurder]+=[info]
                                else:
                                    app.players[uid].information[app.numMurder]=[info]
                        if(uid in app.players):
                            app.players[uid].gravity=friend.position+diff
        ...
        if(app.showChat and (app.lastMessageTime==None or app.secondsPassed-app.lastMessageTime==2)):
            if(app.minUid in app.players): player=app.players[app.minUid]
            else: player=app.villain
            if(app.protagMessage!=None):
                if(player.questionsLeft==1):
                    ...
                    app.otherMessage="That's enough questions for now."
        ...
        if(app.deciseconds%50==0 and app.secondsPassed>=5):
            #checking if it's time to kill
            if(app.killTime or (app.secondsPassed-app.lastMurderTime>=20) or (app.lastMurderTime==-1)):
                ...
                if(app.victimUid==None or app.changed):
                    app.changed=False
                    app.victimUid=decideTheNextKill(app)
                if(app.players[app.victimUid].room==app.villain.room):
                    ...
                    newMurders=app.villain.murders+[newMurder]
                    protag=copy.deepcopy(app.protag)
                    peopleInRoom=getPeopleInRoom(app, app.room)
                    for uid in peopleInRoom:
                        protag.innocents.add(uid)
                    tempPaths=getPathsForPlayerToFindMurderer(app, [], [], app.numMurder, tempPlayers, newMurders, protag)
                    if(app.room!=app.players[app.victimUid].room):
                        if(len(tempPaths)==0):
                            ...
                            if(app.currPathSize==None or (len(minPath)>=app.currPathSize and app.currPathSize!=0) or (len(minPath)>3 and app.currPathSize==0)):
                                app.killTime=True
                                kill(app, app.victimUid)

        #letting players collect information on their surroundings
        if(app.secondsPassed-1!=app.lastMurderTime or app.secondsPassed==1):
            for playerUid in app.players:
                if(app.players[playerUid].alive):
        ...
        for uid in players:
            ...
            if(player.alive):
                if(player.position-app.radius<=event.x<=player.position+app.radius and
                    app.ground-60-(2*app.radius)-1<=event.y<=app.ground-60-1 and
...
def mousePressed(app, event):
    if(not app.newGame):
        ...
        #checking if clicked on yes restart game
        if(not app.showChat and app.showSettings and 
        ...
        if(not app.paused):
            ...
            if(app.showChat):
                ...
                if((app.width/2)-(app.chatWidth/2)+10<=event.x<=(app.width/2)+(app.chatWidth/2)-10 and 
                    (app.height/2)+(app.chatHeight/2)-50<=event.y<=(app.height/2)+(app.chatHeight/2)-10 and app.numMurder>0):
        ...
        if(app.instructions):
            if(20<=event.x<=40 and 20<=event.y<=40):
                app.instructions=False
        else:
            if((app.width/2)-80<=event.x<=(app.width/2)+80 and app.height/2<=event.y<=(app.height/2)+40):
                app.newGame=False
                app.paused=False
            
            #canvas.create_rectangle((app.width/2)-80, (app.height/2)+60, (app.width/2)+80, (app.height/2)+100, fill="darkred")
            if((app.width/2)-80<=event.x<=(app.width/2)+80 and (app.height/2)+60<=event.y<=(app.height/2)+100):
...
def computeResponseToMessage(app, message, playerUid):
    ...
    if(playerUid in app.players):
        ...
        # first, check if anyone has that information & if they do, DO NOT LIE. If not, consider what would happen if that info was added to the user's and if the path to finding out would be longer than the curr path length if i told the truth v the lie
        #checking if murderer should have this information
        mInfo=next((x for x in app.villain.information[message.numMurder] if x.personUid == message.personUid and x.innocent==message.innocent), None)
        if(mInfo!=None):
            # see what would happen if 
            protag=copy.deepcopy(app.protag)
            newInfo=Information(mInfo.numMurder, mInfo.personUid, mInfo.room, mInfo.innocent, mInfo.givenBy)
            protag=updateMockProtagList(app, [newInfo], app.numMurder, False, protag)
            newPaths=getPathsForPlayerToFindMurderer(app, [], [], app.numMurder-1, copy.deepcopy(app.players), copy.deepcopy(app.villain.murders), protag)
            if(len(newPaths)>0):
                minPathLen=len(min(newPaths, key=len))
                # if telling the truth increases path
                if(minPathLen>=app.currPathSize or len(protag.suspects)>=len(app.protag.suspects)):
                    #def updateMockProtagList(app, newInformation, numMurder, murderTime, protag):
                    if(mInfo.innocent==True):
                        text=f"Yes, {app.players[mInfo.personUid].name} is innocent."
                    else:
                        text=f"{app.players[mInfo.personUid].name} was in the {rooms[mInfo.room]} at the time of the murder."
                    updateProtagList(app, [mInfo], app.numMurder, False)
                # if omitting the truth doesn't increase path
                else:
                    text="I do not have any information on that."
            else:
                if(mInfo.innocent==True):
                    text=f"Yes, {app.players[mInfo.personUid].name} is innocent."
                else:
...
def drawPerson(app, canvas, person):
    ...
    if(person.uid==app.protag.uid or person.uid==app.villain.uid or person.alive):
        if(person.highlight):
            canvas.create_oval(positionX-app.radius, app.ground-60-(2*app.radius)-1, 
                positionX+app.radius, app.ground-60-1, fill=person.color, outline="black", width=2)
        else:
            canvas.create_oval(positionX-app.radius, app.ground-60-(2*app.radius)-1, 
                positionX+app.radius, app.ground-60-1, fill=person.color, outline=person.color)
        if(person.uid in app.players and person.smiling):
            canvas.create_arc(positionX-10, app.ground-(2*app.radius)-1+10-60, 
                positionX+10, app.ground-1-10-60, start=180, extent=180, fill="black")
        canvas.create_text(positionX, app.ground-60-(2*app.radius)-11, text=person.name)
        canvas.create_line(positionX, app.ground-60-1, positionX, app.ground-1-15, fill=person.color)
        canvas.create_line(positionX, app.ground-1-15, positionX-15, app.ground-1, fill=person.color)
        canvas.create_line(positionX, app.ground-1-15, positionX+15, app.ground-1, fill=person.color)
        canvas.create_line(positionX, app.ground-1-50, positionX-15, app.ground-1-35, fill=person.color)
        canvas.create_line(positionX, app.ground-1-50, positionX+15, app.ground-1-35, fill=person.color)
    # draw death
    if(person.uid in app.players and not person.alive):
        ...
        canvas.create_line(positionX+app.radius, app.ground-app.radius-1, positionX+app.radius+60, app.ground-app.radius-1, fill=person.color)
        canvas.create_line(positionX+app.radius+60, app.ground-app.radius-1, positionX+app.radius+60+15, app.ground-app.radius-1-15, fill=person.color)
        canvas.create_line(positionX+app.radius+60, app.ground-app.radius-1, positionX+app.radius+60+15, app.ground-1, fill=person.color) 
        canvas.create_line(positionX+app.radius+10, app.ground-app.radius-1, positionX+app.radius+10+15, app.ground-app.radius-1-15, fill=person.color)
        canvas.create_line(positionX+app.radius+10, app.ground-app.radius-1, positionX+app.radius+10+15, app.ground-1, fill=person.color) 
...
def drawChatBubbles(app, canvas):
    ...
    if(app.protagMessage!=None):
        canvas.create_line(positionMe, ground-(2*app.radius)-15-50, positionMe+5, ground-(2*app.radius)-50-50)
        canvas.create_line(positionMe, ground-(2*app.radius)-15-50, positionMe+20, ground-(2*app.radius)-35-50)
        canvas.create_line(positionMe+5, ground-(2*app.radius)-50-50, positionMe+5, ground-(2*app.radius)-100-50)
        canvas.create_line(positionMe+5, ground-(2*app.radius)-100-50, positionMe+355, ground-(2*app.radius)-100-50)
        canvas.create_line(positionMe+355, ground-(2*app.radius)-100-50, positionMe+355, ground-(2*app.radius)-35-50)
        canvas.create_line(positionMe+355, ground-(2*app.radius)-35-50, positionMe+20, ground-(2*app.radius)-35-50)
        text=""
        ...
        if(app.protagMessage.innocent==True):
            ...
            text=f"Where was {player.name} at the time of the last murder?"
        canvas.create_text(positionMe+(355/2), ground-(2*app.radius)-80-40, text=text)
    elif(app.otherMessage!=None):
        if(app.minUid in app.players): player=app.players[app.minUid]
        else: player=app.villain
        canvas.create_line(positionOther, ground-(2*app.radius)-15-50, positionOther-5, ground-(2*app.radius)-50-50)
        canvas.create_line(positionOther, ground-(2*app.radius)-15-50, positionOther-20, ground-(2*app.radius)-35-50)
        canvas.create_line(positionOther-5, ground-(2*app.radius)-50-50, positionOther-5, ground-(2*app.radius)-100-50)
        canvas.create_line(positionOther-5, ground-(2*app.radius)-100-50, positionOther-355, ground-(2*app.radius)-100-50)
        canvas.create_line(positionOther-355, ground-(2*app.radius)-100-50, positionOther-355, ground-(2*app.radius)-35-50)
        canvas.create_line(positionOther-355, ground-(2*app.radius)-35-50, positionOther-20, ground-(2*app.radius)-35-50)
        canvas.create_text(positionOther-(355/2), ground-(2*app.radius)-80-40, text=app.otherMessage)
...
def drawChatInterface(app, canvas):
    ...
    canvas.create_oval(positionMe-app.radius, ground-60-(2*app.radius)-1, 
                positionMe+app.radius, ground-60-1, fill=app.protag.color, outline=app.protag.color)
    canvas.create_text(positionMe, ground-60-(2*app.radius)-11, text=app.protag.name)
    canvas.create_line(positionMe, ground-60-1, positionMe, ground-1-15, fill=app.protag.color)
    canvas.create_line(positionMe, ground-1-15, positionMe-15, ground-1, fill=app.protag.color)
    canvas.create_line(positionMe, ground-1-15, positionMe+15, ground-1, fill=app.protag.color)
    canvas.create_line(positionMe, ground-1-50, positionMe-15, ground-1-35, fill=app.protag.color)
    canvas.create_line(positionMe, ground-1-50, positionMe+15, ground-1-35, fill=app.protag.color)

    positionOther=(app.width/2)+(app.chatWidth/2)-80
    canvas.create_oval(positionOther-app.radius, ground-60-(2*app.radius)-1, 
                positionOther+app.radius, ground-60-1, fill=player.color, outline=player.color)
    canvas.create_text(positionOther, ground-60-(2*app.radius)-11, text=player.name)
    canvas.create_line(positionOther, ground-60-1, positionOther, ground-1-15)
    canvas.create_line(positionOther, ground-1-15, positionOther-15, ground-1, fill=player.color)
    canvas.create_line(positionOther, ground-1-15, positionOther+15, ground-1, fill=player.color)
    canvas.create_line(positionOther, ground-1-50, positionOther-15, ground-1-35, fill=player.color)
    canvas.create_line(positionOther, ground-1-50, positionOther+15, ground-1-35, fill=player.color)
    ...
    canvas.create_text((app.width/2), (app.height/2)+(app.chatHeight/2)-30, text="ASK ABOUT SOMEONE ELSE'S POSITION DURING LAST MURDER", fill="white")
...
def drawReportMurdererInterface(app, canvas):
    ...
    for player in players:
        radius=30
        if(index<5):
            canvas.create_oval(positionX-radius+(difference*index), ground1-(2*radius), 
                positionX+radius+(difference*index), ground1, fill=player.color, outline=player.color)
            canvas.create_text(positionX+(difference*index), ground1+10, text=player.name)
            if(player.eliminated or not player.alive):
                canvas.create_image(positionX+(difference*index), ground1-radius, image=ImageTk.PhotoImage(app.deadImage))
        else:
            canvas.create_oval(positionX-radius+(difference*(index-5)), ground2-(2*radius), 
                positionX+radius+(difference*(index-5)), ground2, fill=player.color, outline=player.color)
            canvas.create_text(positionX+(difference*(index-5)), ground2+10, text=player.name)
            if(player.eliminated or not player.alive):
                canvas.create_image(positionX+(difference*(index-5)), ground2-radius, image=ImageTk.PhotoImage(app.deadImage))
        index+=1
...
def drawNewMurderInterface(app, canvas):
    ...
    canvas.create_text(app.width/2, (app.height/2)-(app.settingsHeight/2)+30, 
        text=f"{app.players[app.villain.murders[len(app.villain.murders)-1].victim].name} was killed!")
    canvas.create_rectangle((app.width/2)-(app.settingsWidth/2), (app.height/2)+(app.settingsHeight/2)-40, 
        (app.width/2)+(app.settingsWidth/2), (app.height/2)+(app.settingsHeight/2), fill="indigo")
    canvas.create_text((app.width/2), 
        (app.height/2)+(app.settingsHeight/2)-20, fill="white", text="CONTINUE")

def drawNewGameScreen(app, canvas):
    canvas.create_rectangle(0, 0, app.width, app.width, fill="black")
    canvas.create_text(app.width/2, app.height/2-50, text="Whodunnit?", font="Helvetica 30", fill="mediumslateblue")
    canvas.create_rectangle((app.width/2)-80, app.height/2, (app.width/2)+80, (app.height/2)+40, fill="darkred")
    canvas.create_text(app.width/2, (app.height/2)+20, text="START GAME", fill="white")
    canvas.create_rectangle((app.width/2)-80, (app.height/2)+60, (app.width/2)+80, (app.height/2)+100, fill="darkred")
    canvas.create_text(app.width/2, (app.height/2)+80, text="INSTRUCTIONS", fill="white")
    canvas.create_image(app.width-60, app.height-60, image=ImageTk.PhotoImage(app.cakeImage))
    canvas.create_image(100, 100, image=ImageTk.PhotoImage(app.bloodImage))

def drawInstructions(app, canvas):
    canvas.create_rectangle(0, 0, app.width, app.width, fill="black")
    canvas.create_text(app.width/2, 40, text="Instructions", font="Helvetica 20", fill="mediumslateblue")
    canvas.create_text(app.width/2, 90, fill="white", text="You're attending a birthday party with seven other people. However, one of them begins turning\n the lights off and killing someone in the confusion.")
    canvas.create_text(app.width/2, 130, fill="white", text="1. You must identify who the murderer is before everyone else is killed. ")
    canvas.create_text(app.width/2, 170, fill="white", text="2. You can ask other people questions about where other people were at the time of the murder.")
    canvas.create_text(app.width/2, 290, fill="white", text="3. Some people may be distracted and not collect all information in their room.")
    canvas.create_text(app.width/2, 210, fill="white", text="4. Some people may show that they're innocent by smiling--murderers cannot smile! Be on the lookout.")
    canvas.create_text(app.width/2, 250, fill="white", text="5. When you deduce who the murderer is, you can select the person. You have one try.")
    canvas.create_image(30, 30, image=ImageTk.PhotoImage(app.backImage))
...
def redrawAll(app, canvas):
    ...
    canvas.create_image(app.width/2, 90, image=ImageTk.PhotoImage(app.bannerImage))
