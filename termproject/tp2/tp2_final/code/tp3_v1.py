...
def appStarted(app):
    ...
    app.players[app.playersToBeInnocent[0]].smileAfterMurderNumber=random.randint(1, 4)
    app.players[app.playersToBeInnocent[1]].smileAfterMurderNumber=random.randint(1, 4)
...
def movePlayerTowardsRoom(app, player):
    if(player.room!=player.gravityRoom):
        if(player.position<=app.border+app.radius):
            if(player.room!=0):
                player.room-=1
                player.position=app.width-app.border-app.radius-2
        elif(player.position>=app.width-app.border-app.radius):
            if(player.room!=4):
                player.room+=1
                player.position=app.border+app.radius+2.5
        else:
...
def generateRandomMap(app):
    cols=random.randint(3, 6)
    rows=6-cols
    randomMap=make2dList(rows, cols, -1)
    roomsLeft=copy.deepcopy(rooms)
    ...
    if(not app.paused and app.pressed and not app.showChat):
        ...
        if(app.mouseY>=app.border+100):
            if(app.mouseX>app.protag.position and 
                ...
                app.protag.position+=(difference/120)*5
        ...
        if(app.deciseconds%50==0 and app.secondsPassed>=5):
            ...
            if(app.killTime or (app.secondsPassed-app.lastMurderTime>=20) or (app.lastMurderTime==-1)):
                ...
                if(app.players[app.victimUid].room==app.villain.room):
                    ...
                    newMurders=app.villain.murders+[newMurder]
                    tempPaths=getPathsForPlayerToFindMurderer(app, [], [], app.numMurder, tempPlayers, newMurders, app.protag)
...
def mousePressed(app, event):
    if(not app.newGame):
        ...
        if(not app.paused):
            ...
            if(app.showMap and eventInBoundaries(event.x, event.y, 
                (app.width/2)-(app.chatWidth/2)+25, (app.height/2)-(app.chatHeight/2)+25)):
                app.showMap=False
...
def drawPerson(app, canvas, person):
    ...
    if(person.uid==app.protag.uid or person.uid==app.villain.uid or person.alive):
        ...
        canvas.create_line(positionX, app.ground-60-1, positionX, app.ground-1-15)
        canvas.create_line(positionX, app.ground-1-15, positionX-15, app.ground-1)
        canvas.create_line(positionX, app.ground-1-15, positionX+15, app.ground-1)
        canvas.create_line(positionX, app.ground-1-50, positionX-15, app.ground-1-35)
        canvas.create_line(positionX, app.ground-1-50, positionX+15, app.ground-1-35)
    # draw death
    if(person.uid in app.players and not person.alive):
        ...
        canvas.create_line(positionX+app.radius, app.ground-app.radius-1, positionX+app.radius+60, app.ground-app.radius-1)
        canvas.create_line(positionX+app.radius+60, app.ground-app.radius-1, positionX+app.radius+60+15, app.ground-app.radius-1-15)
        canvas.create_line(positionX+app.radius+60, app.ground-app.radius-1, positionX+app.radius+60+15, app.ground-1) 
        canvas.create_line(positionX+app.radius+10, app.ground-app.radius-1, positionX+app.radius+10+15, app.ground-app.radius-1-15)
        canvas.create_line(positionX+app.radius+10, app.ground-app.radius-1, positionX+app.radius+10+15, app.ground-1) 
...
def drawChatBubbles(app, canvas):
    ...
    if(app.protagMessage!=None):
        ...
        canvas.create_line(positionMe+5, ground-(2*app.radius)-125, positionMe+355, ground-(2*app.radius)-125)
        canvas.create_line(positionMe+355, ground-(2*app.radius)-125, positionMe+355, ground-(2*app.radius)-35)
        canvas.create_line(positionMe+355, ground-(2*app.radius)-35, positionMe+20, ground-(2*app.radius)-35)
        text=""
        ...
        canvas.create_text(positionMe+(355/2), ground-(2*app.radius)-80, text=text)
    elif(app.otherMessage!=None):
        ...
        canvas.create_line(positionOther-5, ground-(2*app.radius)-125, positionOther-355, ground-(2*app.radius)-125)
        canvas.create_line(positionOther-355, ground-(2*app.radius)-125, positionOther-355, ground-(2*app.radius)-35)
        canvas.create_line(positionOther-355, ground-(2*app.radius)-35, positionOther-20, ground-(2*app.radius)-35)
        if(player.questionsLeft==0):
            canvas.create_text(positionOther-(355/2), ground-(2*app.radius)-80, text="That's enough questions for now.")
        else:
            canvas.create_text(positionOther-(355/2), ground-(2*app.radius)-80, text=app.otherMessage)
...
def drawMapInterface(app, canvas):
    canvas.create_rectangle((app.width/2)-(app.chatWidth/2), (app.height/2)-(app.chatHeight/2), 
        (app.width/2)+(app.chatWidth/2), (app.height/2)+(app.chatHeight/2), fill="white")
    canvas.create_rectangle((app.width/2)-(app.chatWidth/2), (app.height/2)-(app.chatHeight/2), 
        (app.width/2)+(app.chatWidth/2), (app.height/2)-(app.chatHeight/2)+50, fill="indigo")
    canvas.create_text(app.width/2, (app.height/2)-(app.chatHeight/2)+25, 
        text=f"MAP", fill="white", font="Helvetica 15")
    canvas.create_image((app.width/2)-(app.chatWidth/2)+25, (app.height/2)-(app.chatHeight/2)+25, 
        image=ImageTk.PhotoImage(app.closeImage))
...
def drawInstructions(app, canvas):
    ...
    canvas.create_text(app.width/2, 210, fill="white", text="3. Some people may show that they're innocent by smiling--murderers cannot smile! Be on the lookout.")
    canvas.create_text(app.width/2, 250, fill="white", text="4. When you deduce who the murderer is, you can select the person. You have one try.")
