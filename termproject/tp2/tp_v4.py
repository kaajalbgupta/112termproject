...
def updateProtagList(app, newInformation, numMurder, murderTime):
    ...
    for info in newInformation:
        ...
        if(player.alive and not player.eliminated):
            ...
            # if someone says a person isn't in this room
    ...
    #Recursive Case
        ...
        for uid in players:
            if(players[uid].alive and uid not in playersDone):
                ...
                if(len(players[uid].information)>0 and currMurder in players[uid].information):
...
def playerCollectInformation(app, playerUid, isMurderTime):
    ...
    if(len(peopleInRoom)!=1):
        ...
        for personUid in peopleInRoom:
            if(personUid!=playerUid):
                ...
                if(isMurderTime):
                    if(person.smiling):
                        info=Information(app.numMurder, personUid, room, True, playerUid, True)
                        events+=[info]
                    elif(not person.smiling):
                        info=Information(app.numMurder, personUid, room, None, playerUid, True)
                        events+=[info]
                else:
                    if(person.smiling):
                        info=Information(app.numMurder, personUid, room, True, playerUid, True)
    ...
    #adding information about their own location
    eventsRecorded+=[Information(app.numMurder, playerUid, room, None, playerUid, True)]
...
def mockPlayersCollectInformation(app):
    ...
    for playerUid in players:
        ...
        playersWithVillain=list(app.players.keys())+[app.villain.uid]
        ...
        if(len(peopleInRoom)!=1):
            ...
            for personUid in peopleInRoom:
                ...
                if(person.smiling and personUid!=playerUid):
                    info=Information(app.numMurder, personUid, room, True, playerUid, True)
                    events+=[info]
                elif(not person.smiling):
                    info=Information(app.numMurder, personUid, room, None, playerUid, True)
...
def timerFired(app):
    ...
    if(not app.paused):
        ...
        if(app.deciseconds%50==0):
            ...
            if(app.secondsPassed-1==app.lastMurderTime and app.secondsPassed!=0):
...
def mousePressed(app, event):
    ...
    if(not app.paused):
        ...
        if(app.showChat):
            ...
            if((app.width/2)-(app.chatWidth/2)+10<=event.x<=(app.width/2)-5 and 
                ...
                if(name!=None):
                    ...
                    app.protagMessage=Information(app.numMurder,playerUid, -1, True, None, True)
            
            # checking if their location button was clicked
            if((app.width/2)+5<=event.x<=(app.width/2)+(app.chatWidth/2)-10 and 
                (app.height/2)+(app.chatHeight/2)-100<=event.y<=(app.height/2)+(app.chatHeight/2)-60):
                app.protagMessage=Information(app.numMurder,app.minUid, -1, None, None, True)
            ...
            if((app.width/2)-(app.chatWidth/2)+10<=event.x<=(app.width/2)+(app.chatWidth/2)-10 and 
                ...
                if(name!=None):
                    playerUid=getPlayerByName(app, name)
                    app.protagMessage=Information(app.numMurder,playerUid, -1, None, None, True)
    ...
    if(playerUid in app.players):
        ...
        if(info!=None):
            if(info.innocent==True):
                ...
                if(info.personUid==app.minUid):
                    text=f"I was in the {rooms[info.room]} at the time of the murder."
                else:
                    if(not info.presentInRoom):
                        text=f"{players[info.personUid].name} was not in the {rooms[info.room]} at the time of the murder."
                    else: text=f"{players[info.personUid].name} was in the {rooms[info.room]} at the time of the murder."
        ...
        for uid in app.players:
            player=app.players[uid]
            informationCurrMurder=player.information[message.numMurder]
            otherInfo=next((x for x in informationCurrMurder if x.personUid == message.personUid and x.innocent==message.innocent), None)
            if(otherInfo!=None):
                break
        if(otherInfo!=None):
            #checking if murderer should have this information
            mInfo=next((x for x in app.villain.information[message.numMurder] if x.personUid == otherInfo.personUid and x.innocent==otherInfo.innocent), None)
            # to get villain's room at time of murder
            myInfo=next((x for x in app.villain.information[message.numMurder] if x.personUid == app.villain.uid), None)
            if(mInfo!=None and not mInfo.innocent):
                # see what would happen if tell the truth
                protag=copy.deepcopy(app.protag)
                newInfo=Information(mInfo.numMurder, mInfo.personUid, mInfo.room, mInfo.innocent, mInfo.givenBy, mInfo.presentInRoom)
                protag=updateMockProtagList(app, newInfo, app.numMurder, False, protag)
                newPaths=getPathsForPlayerToFindMurderer(app, [], [], app.numMurder, copy.deepcopy(app.players), copy.deepcopy(app.villain.murders), protag)

                if(len(newPaths)>0):
                    minPathLen=len(min(newPaths, key=len))

                    # see what would happen if tell a lie
                    protag=copy.deepcopy(app.protag)
                    falseInfo=Information(mInfo.numMurder, mInfo.personUid, myInfo.room, None, mInfo.givenBy, not mInfo.presentInRoom)
                    protag=updateMockProtagList(app, falseInfo, app.numMurder, False, protag)
                    newFalsePaths=getPathsForPlayerToFindMurderer(app, [], [], app.numMurder, copy.deepcopy(app.players), copy.deepcopy(app.villain.murders), protag)
                    
                    # if telling the truth increases path
                    if(len(newFalsePaths)>0):
                        minFalsePathLen=len(min(newFalsePaths, key=len))
                        maxLenPath=max([minPathLen, minFalsePathLen, app.currPathSize])
                        if(maxLenPath==minPathLen):
                            if(not newInfo.presentInRoom):
                                text=f"No, {players[newInfo.personUid].name} was not in my room at the time of the murder."
                            else:
                                text=f"{players[newInfo.personUid].name} was in the {rooms[newInfo.room]} at the time of the murder."
                            updateProtagList(app, [newInfo], app.numMurder, False)
                        elif(maxLenPath==minFalsePathLen):
                            if(not falseInfo.presentInRoom):
                                text=f"No, {players[falseInfo.personUid].name} was not in my room at the time of the murder."
                            else:
                                text=f"{players[falseInfo.personUid].name} was in the {rooms[falseInfo.room]} at the time of the murder."
                            updateProtagList(app, [falseInfo], app.numMurder, False)
                        else:
                ...
                # if there are no ways to figure it out, say the info
                else:
                    if(not newInfo.presentInRoom):
                        text=f"No, {players[newInfo.personUid].name} was not in my room at the time of the murder."
                    else:
...
def drawChatBubbles(app, canvas):
    ...
    if(app.protagMessage!=None):
        ...
        if(app.protagMessage.innocent==True):
            ...
            if(app.protagMessage.personUid==app.minUid):
...
def drawChatInterface(app, canvas):
    ...
    canvas.create_rectangle((app.width/2)-(app.chatWidth/2)+10, (app.height/2)+(app.chatHeight/2)-100, 
        (app.width/2)-5, (app.height/2)+(app.chatHeight/2)-60, fill="teal", outline="teal")
    canvas.create_text((app.width/2), (app.height/2)+(app.chatHeight/2)-80, 
        text="ASK ABOUT SOMEONE'S INNOCENCE", font="Helvetica 10", fill="white")
    canvas.create_rectangle((app.width/2)+5, (app.height/2)+(app.chatHeight/2)-100, 
        (app.width/2)+(app.chatWidth/2)-10, (app.height/2)+(app.chatHeight/2)-60, fill="teal", outline="teal")
    canvas.create_text((app.width/2), (app.height/2)+(app.chatHeight/2)-80, 
        text="ASK ABOUT THEIR LOCATION DURING LAST MURDER", font="Helvetica 10", fill="white")
    canvas.create_rectangle((app.width/2)-(app.chatWidth/2)+10, (app.height/2)+(app.chatHeight/2)-50, 
        (app.width/2)+(app.chatWidth/2)-10, (app.height/2)+(app.chatHeight/2)-10, fill="indigo", outline="indigo")
    canvas.create_text((app.width/2), (app.height/2)+(app.chatHeight/2)-30, text="ASK ABOUT SOMEONE ELSE'S LOCATION DURING LAST MURDER", fill="white")
