...
def appStarted(app):
    ...
    app.playersToBeInnocent=random.sample(app.players.keys(), 2)
...
def updateProtagList(app, newInformation, numMurder):
    ...
    for info in newInformation:
        if(app.players[info.personUid].alive and not app.players[info.personUid].eliminated):
...
def checkIfInfoRevealsMurderer(app, knownInformation, numMurder, players):
    murder=app.villain.murders[numMurder]
...
def getPathsForPlayerToFindMurderer(app, currInfo, playersDone, currMurder, players):
    #Base Case
    if(checkIfInfoRevealsMurderer(app, currInfo, currMurder, players)):
        return playersDone
        ...
        for uid in players:
            if(players[uid].alive and uid not in playersDone):
                ...
                for info in information:
                    ...
                    returned=getPathsForPlayerToFindMurderer(app, newCurrInfo, newPlayersDone, currMurder, players)
                    if(returned!=None):
...
# if same length path, kill the most talkative person
    ...
    for uid in players:
        if(players[uid].alive):
            ...
            paths=getPathsForPlayerToFindMurderer(app, [], 
                [], len(app.villain.murders)-1, players)
            # get min size path and compare to overall min size path
            ...
            if(minPathSize==None or currMinPathSize>=minPathSize):
                if(currMinPathSize==minPathSize):
                    if(players[uid].characteristics.talkativeness>players[uidToKill].characteristics.talkativeness):
            ...
            if(not player.stationary and player.alive):
                ...
                if(app.secondsPassed%(20+(index*2))==0):
                    ...
                    while(num==player.currFriendIndex):
                        num=random.randint(0, len(player.friends)+len(player.friends)//2)
...
def mousePressed(app, event):
    ...
    if(app.showReportMurderer):
        ...
        if(clickedPlayerUid!=None):
            print(app.players[clickedPlayerUid].name)
...
def drawChatBubbles(app, canvas):
    ...
    if(app.protagMessage!=""):
        ...
        canvas.create_line(positionMe+5, ground-(2*app.radius)-125, positionMe+205, ground-(2*app.radius)-125)
        canvas.create_line(positionMe+205, ground-(2*app.radius)-125, positionMe+205, ground-(2*app.radius)-35)
        canvas.create_line(positionMe+205, ground-(2*app.radius)-35, positionMe+20, ground-(2*app.radius)-35)
        canvas.create_text(positionMe+105, ground-(2*app.radius)-80, text=app.protagMessage)
    elif(app.otherMessage!=""):
        if(app.players[app.minUid].questionsLeft==0):
            pass
        else:
            pass
